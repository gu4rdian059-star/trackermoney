...
const mapRowToTx = (row: any): Transaction => {
  return {
    ...
    walletId: row.wallet_id || 'cash',
    walletName: row.wallet_name || 'Tunai / Dompet',
...
export const financeStorage = {
  ...
  async addTransaction(payload: Omit<Transaction, 'id' | 'createdAt'>): Promise<Transaction> {
    ...
    const newTx: Transaction = {
      ...payload,
      walletId: payload.walletId || 'cash',
      walletName: payload.walletName || 'Tunai / Dompet',
      walletType: payload.walletType || 'cash',
      receiptUri: payload.receiptUri || undefined,
    ...
    try {
      await supabase.from('transactions').insert({
        ...
        wallet_name: newTx.walletName,
        wallet_type: newTx.walletType,
        receipt_uri: newTx.receiptUri || null,
        date: newTx.date,
